...
pub trait EscrowVariant {
    ...
    ) -> impl std::future::Future<Output = u64> + Send; // async fn get_escrow_rent(test_state: &mut TestStateBase<Self>)
...
impl<T> TestStateBase<T> {
    ...
    pub async fn get_ata_rent(&mut self) -> u64 {
        RENT_FOR_ATA
            .get_or_init(|| async {
                get_min_rent_for_size(&mut self.client, SplTokenAccount::LEN).await
            })
            .await
            .to_owned()
    }
